//! Rust [`serde::Serializer`]/[`serde::Deserializer`] corresponding to C++ class `hkpSphereMotion`
//!
//! # NOTE
...
impl Serialize for HkpSphereMotion<'_> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        // Use `Vec` instead, because the fields of this class are more than 32 and serde only supports up to `[T; 32]`.
        let visitor: Vec<HkpSphereMotionVisitor<'_>> = self.into();
        visitor.serialize(serializer)
    }
}

impl<'de> Deserialize<'de> for HkpSphereMotion<'de> {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        // Use `Vec` instead, because the fields of this class are more than 32 and serde only supports up to `[T; 32]`.
        let de = <Vec<HkpSphereMotionVisitor<'de>>>::deserialize(deserializer)?;
        Ok(de.into())
    }
}

impl<'a> From<Vec<HkpSphereMotionVisitor<'a>>> for HkpSphereMotion<'a> {
    fn from(_values: Vec<HkpSphereMotionVisitor<'a>>) -> Self {
            let mut _type = None;
        ...
        for _value in _values {
            match _value {
                HkpSphereMotionVisitor::Type(m) => _type = Some(m),
                HkpSphereMotionVisitor::DeactivationIntegrateCounter(m) => deactivation_integrate_counter = Some(m),
                HkpSphereMotionVisitor::DeactivationNumInactiveFrames(m) => deactivation_num_inactive_frames = Some(m),
                HkpSphereMotionVisitor::MotionState(m) => motion_state = Some(m),
                HkpSphereMotionVisitor::InertiaAndMassInv(m) => inertia_and_mass_inv = Some(m),
                HkpSphereMotionVisitor::LinearVelocity(m) => linear_velocity = Some(m),
                HkpSphereMotionVisitor::AngularVelocity(m) => angular_velocity = Some(m),
                HkpSphereMotionVisitor::DeactivationRefPosition(m) => deactivation_ref_position = Some(m),
                HkpSphereMotionVisitor::DeactivationRefOrientation(m) => deactivation_ref_orientation = Some(m),
                HkpSphereMotionVisitor::SavedMotion(m) => saved_motion = Some(m),
                HkpSphereMotionVisitor::SavedQualityTypeIndex(m) => saved_quality_type_index = Some(m),
                HkpSphereMotionVisitor::GravityFactor(m) => gravity_factor = Some(m),
                HkpSphereMotionVisitor::MemSizeAndFlags(m) => mem_size_and_flags = Some(m),
                HkpSphereMotionVisitor::ReferenceCount(m) => reference_count = Some(m),
...
impl<'a> From<&HkpSphereMotion<'a>> for Vec<HkpSphereMotionVisitor<'a>> {
    fn from(data: &HkpSphereMotion<'a>) -> Self {
        vec![
            HkpSphereMotionVisitor::Type(data._type.clone().into()),
            HkpSphereMotionVisitor::DeactivationIntegrateCounter(data.deactivation_integrate_counter.into()),
            HkpSphereMotionVisitor::DeactivationNumInactiveFrames(data.deactivation_num_inactive_frames.clone()),
            HkpSphereMotionVisitor::MotionState(data.motion_state.clone()),
            HkpSphereMotionVisitor::InertiaAndMassInv(data.inertia_and_mass_inv.into()),
            HkpSphereMotionVisitor::LinearVelocity(data.linear_velocity.into()),
            HkpSphereMotionVisitor::AngularVelocity(data.angular_velocity.into()),
            HkpSphereMotionVisitor::DeactivationRefPosition(data.deactivation_ref_position.clone()),
            HkpSphereMotionVisitor::DeactivationRefOrientation(data.deactivation_ref_orientation.clone()),
            HkpSphereMotionVisitor::SavedMotion(data.saved_motion.clone().into()),
            HkpSphereMotionVisitor::SavedQualityTypeIndex(data.saved_quality_type_index.into()),
            HkpSphereMotionVisitor::GravityFactor(data.gravity_factor.into()),
            HkpSphereMotionVisitor::MemSizeAndFlags(data.mem_size_and_flags.into()),
            HkpSphereMotionVisitor::ReferenceCount(data.reference_count.into()),

        ]
    }
}

impl <'bytes: 'de, 'de> ByteDeSerialize<'bytes, 'de> for HkpSphereMotion<'de> {
    fn from_bytes<B>(
        _bytes: &'bytes [u8],
...
impl_deserialize_for_internally_tagged_enum! {
    HkpSphereMotionVisitor<'de>, "@name",
