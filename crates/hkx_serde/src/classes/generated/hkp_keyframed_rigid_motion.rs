//! Rust [`serde::Serializer`]/[`serde::Deserializer`] corresponding to C++ class `hkpKeyframedRigidMotion`
//!
//! # NOTE
...
impl Serialize for HkpKeyframedRigidMotion<'_> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        // Use `Vec` instead, because the fields of this class are more than 32 and serde only supports up to `[T; 32]`.
        let visitor: Vec<HkpKeyframedRigidMotionVisitor<'_>> = self.into();
        visitor.serialize(serializer)
    }
}

impl<'de> Deserialize<'de> for HkpKeyframedRigidMotion<'de> {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        // Use `Vec` instead, because the fields of this class are more than 32 and serde only supports up to `[T; 32]`.
        let de = <Vec<HkpKeyframedRigidMotionVisitor<'de>>>::deserialize(deserializer)?;
        Ok(de.into())
    }
}

impl<'a> From<Vec<HkpKeyframedRigidMotionVisitor<'a>>> for HkpKeyframedRigidMotion<'a> {
    fn from(_values: Vec<HkpKeyframedRigidMotionVisitor<'a>>) -> Self {
            let mut _type = None;
        ...
        for _value in _values {
            match _value {
                HkpKeyframedRigidMotionVisitor::Type(m) => _type = Some(m),
                HkpKeyframedRigidMotionVisitor::DeactivationIntegrateCounter(m) => deactivation_integrate_counter = Some(m),
                HkpKeyframedRigidMotionVisitor::DeactivationNumInactiveFrames(m) => deactivation_num_inactive_frames = Some(m),
                HkpKeyframedRigidMotionVisitor::MotionState(m) => motion_state = Some(m),
                HkpKeyframedRigidMotionVisitor::InertiaAndMassInv(m) => inertia_and_mass_inv = Some(m),
                HkpKeyframedRigidMotionVisitor::LinearVelocity(m) => linear_velocity = Some(m),
                HkpKeyframedRigidMotionVisitor::AngularVelocity(m) => angular_velocity = Some(m),
                HkpKeyframedRigidMotionVisitor::DeactivationRefPosition(m) => deactivation_ref_position = Some(m),
                HkpKeyframedRigidMotionVisitor::DeactivationRefOrientation(m) => deactivation_ref_orientation = Some(m),
                HkpKeyframedRigidMotionVisitor::SavedMotion(m) => saved_motion = Some(m),
                HkpKeyframedRigidMotionVisitor::SavedQualityTypeIndex(m) => saved_quality_type_index = Some(m),
                HkpKeyframedRigidMotionVisitor::GravityFactor(m) => gravity_factor = Some(m),
                HkpKeyframedRigidMotionVisitor::MemSizeAndFlags(m) => mem_size_and_flags = Some(m),
                HkpKeyframedRigidMotionVisitor::ReferenceCount(m) => reference_count = Some(m),
...
impl<'a> From<&HkpKeyframedRigidMotion<'a>> for Vec<HkpKeyframedRigidMotionVisitor<'a>> {
    fn from(data: &HkpKeyframedRigidMotion<'a>) -> Self {
        vec![
            HkpKeyframedRigidMotionVisitor::Type(data._type.clone().into()),
            HkpKeyframedRigidMotionVisitor::DeactivationIntegrateCounter(data.deactivation_integrate_counter.into()),
            HkpKeyframedRigidMotionVisitor::DeactivationNumInactiveFrames(data.deactivation_num_inactive_frames.clone()),
            HkpKeyframedRigidMotionVisitor::MotionState(data.motion_state.clone()),
            HkpKeyframedRigidMotionVisitor::InertiaAndMassInv(data.inertia_and_mass_inv.into()),
            HkpKeyframedRigidMotionVisitor::LinearVelocity(data.linear_velocity.into()),
            HkpKeyframedRigidMotionVisitor::AngularVelocity(data.angular_velocity.into()),
            HkpKeyframedRigidMotionVisitor::DeactivationRefPosition(data.deactivation_ref_position.clone()),
            HkpKeyframedRigidMotionVisitor::DeactivationRefOrientation(data.deactivation_ref_orientation.clone()),
            HkpKeyframedRigidMotionVisitor::SavedMotion(data.saved_motion.clone().into()),
            HkpKeyframedRigidMotionVisitor::SavedQualityTypeIndex(data.saved_quality_type_index.into()),
            HkpKeyframedRigidMotionVisitor::GravityFactor(data.gravity_factor.into()),
            HkpKeyframedRigidMotionVisitor::MemSizeAndFlags(data.mem_size_and_flags.into()),
            HkpKeyframedRigidMotionVisitor::ReferenceCount(data.reference_count.into()),
...
impl_deserialize_for_internally_tagged_enum! {
    HkpKeyframedRigidMotionVisitor<'de>, "@name",
