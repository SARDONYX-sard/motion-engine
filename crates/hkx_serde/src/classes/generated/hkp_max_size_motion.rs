//! Rust [`serde::Serializer`]/[`serde::Deserializer`] corresponding to C++ class `hkpMaxSizeMotion`
//!
//! # NOTE
...
impl Serialize for HkpMaxSizeMotion<'_> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        // Use `Vec` instead, because the fields of this class are more than 32 and serde only supports up to `[T; 32]`.
        let visitor: Vec<HkpMaxSizeMotionVisitor<'_>> = self.into();
        visitor.serialize(serializer)
    }
}

impl<'de> Deserialize<'de> for HkpMaxSizeMotion<'de> {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        // Use `Vec` instead, because the fields of this class are more than 32 and serde only supports up to `[T; 32]`.
        let de = <Vec<HkpMaxSizeMotionVisitor<'de>>>::deserialize(deserializer)?;
        Ok(de.into())
    }
}

impl<'a> From<Vec<HkpMaxSizeMotionVisitor<'a>>> for HkpMaxSizeMotion<'a> {
    fn from(_values: Vec<HkpMaxSizeMotionVisitor<'a>>) -> Self {
            let mut _type = None;
        ...
        for _value in _values {
            match _value {
                HkpMaxSizeMotionVisitor::Type(m) => _type = Some(m),
                HkpMaxSizeMotionVisitor::DeactivationIntegrateCounter(m) => deactivation_integrate_counter = Some(m),
                HkpMaxSizeMotionVisitor::DeactivationNumInactiveFrames(m) => deactivation_num_inactive_frames = Some(m),
                HkpMaxSizeMotionVisitor::MotionState(m) => motion_state = Some(m),
                HkpMaxSizeMotionVisitor::InertiaAndMassInv(m) => inertia_and_mass_inv = Some(m),
                HkpMaxSizeMotionVisitor::LinearVelocity(m) => linear_velocity = Some(m),
                HkpMaxSizeMotionVisitor::AngularVelocity(m) => angular_velocity = Some(m),
                HkpMaxSizeMotionVisitor::DeactivationRefPosition(m) => deactivation_ref_position = Some(m),
                HkpMaxSizeMotionVisitor::DeactivationRefOrientation(m) => deactivation_ref_orientation = Some(m),
                HkpMaxSizeMotionVisitor::SavedMotion(m) => saved_motion = Some(m),
                HkpMaxSizeMotionVisitor::SavedQualityTypeIndex(m) => saved_quality_type_index = Some(m),
                HkpMaxSizeMotionVisitor::GravityFactor(m) => gravity_factor = Some(m),
                HkpMaxSizeMotionVisitor::MemSizeAndFlags(m) => mem_size_and_flags = Some(m),
                HkpMaxSizeMotionVisitor::ReferenceCount(m) => reference_count = Some(m),
...
impl<'a> From<&HkpMaxSizeMotion<'a>> for Vec<HkpMaxSizeMotionVisitor<'a>> {
    fn from(data: &HkpMaxSizeMotion<'a>) -> Self {
        vec![
            HkpMaxSizeMotionVisitor::Type(data._type.clone().into()),
            HkpMaxSizeMotionVisitor::DeactivationIntegrateCounter(data.deactivation_integrate_counter.into()),
            HkpMaxSizeMotionVisitor::DeactivationNumInactiveFrames(data.deactivation_num_inactive_frames.clone()),
            HkpMaxSizeMotionVisitor::MotionState(data.motion_state.clone()),
            HkpMaxSizeMotionVisitor::InertiaAndMassInv(data.inertia_and_mass_inv.into()),
            HkpMaxSizeMotionVisitor::LinearVelocity(data.linear_velocity.into()),
            HkpMaxSizeMotionVisitor::AngularVelocity(data.angular_velocity.into()),
            HkpMaxSizeMotionVisitor::DeactivationRefPosition(data.deactivation_ref_position.clone()),
            HkpMaxSizeMotionVisitor::DeactivationRefOrientation(data.deactivation_ref_orientation.clone()),
            HkpMaxSizeMotionVisitor::SavedMotion(data.saved_motion.clone().into()),
            HkpMaxSizeMotionVisitor::SavedQualityTypeIndex(data.saved_quality_type_index.into()),
            HkpMaxSizeMotionVisitor::GravityFactor(data.gravity_factor.into()),
            HkpMaxSizeMotionVisitor::MemSizeAndFlags(data.mem_size_and_flags.into()),
            HkpMaxSizeMotionVisitor::ReferenceCount(data.reference_count.into()),

        ]
    }
}

impl <'bytes: 'de, 'de> ByteDeSerialize<'bytes, 'de> for HkpMaxSizeMotion<'de> {
    fn from_bytes<B>(
        _bytes: &'bytes [u8],
...
#[serde(tag = "@name")]
enum HkpMaxSizeMotionVisitor<'a> {
    // C++ Parent class(`hkpKeyframedRigidMotion` => parent: `hkpMotion`) has no fields
    //
    /// Visitor fields
    #[serde(rename = "type")]
...
impl_deserialize_for_internally_tagged_enum! {
    HkpMaxSizeMotionVisitor<'de>, "@name",
