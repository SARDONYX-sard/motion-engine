...
pub struct HkbTwistModifier<'a> {
    ...
    /// - offset: 72
    /// -  flags: `FLAGS_NONE`
    set_angle_method: SetAngleMethod,
    /// # C++ Class Fields Info
    /// -   name:`"rotationAxisCoordinates"`
    /// -   type: `enum RotationAxisCoordinates`
    /// - offset: 73
    /// -  flags: `FLAGS_NONE`
    rotation_axis_coordinates: RotationAxisCoordinates,
    /// # C++ Class Fields Info
    /// -   name:`"isAdditive"`
    /// -   type: `hkBool`
    /// - offset: 74
    ...
    /// - offset: 88
    /// -  flags: `FLAGS_NONE|SERIALIZE_IGNORED`
    parent_bone_indices: HkArrayRef<()>,
}
...
impl ByteDeSerialize for HkbTwistModifier<'_> {
    fn from_bytes<B>(
        _bytes: &[u8],
...
#[serde(tag = "@name")]
pub enum HkbTwistModifierVisitor<'a> {
    /// Visitor fields
    #[serde(rename = "enable")]
