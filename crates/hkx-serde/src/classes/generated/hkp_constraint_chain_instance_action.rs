//! Rust [`Serializer`]/[`Deserializer`] corresponding to C++ class `hkpConstraintChainInstanceAction`
//!
//! # NOTE
//! This file is generated automatically by parsing the rpt files obtained by executing the `hkxcmd Report` command.
use super::*;
use crate::havok_types::*;
use quick_xml::impl_deserialize_for_internally_tagged_enum;
use serde::{Deserialize, Serialize};
use std::borrow::Cow;

/// `hkpConstraintChainInstanceAction`
///
/// - In C++, it represents the name of one field in the class.
/// - In XML, the value of the `name` attribute of the `hkparam` tag.
///
/// # C++ Class Info
/// -      size: 28
/// -    vtable: true
/// -    parent: `hkpAction`/`0xbdf70a51`
/// - signature: `0xc3971189`
/// -   version: 0
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(tag = "@name")]
pub enum HkpConstraintChainInstanceAction<'a> {
    /// # C++ Class Fields Info
    /// -   name:`"constraintInstance"`
    /// -   type: `struct hkpConstraintChainInstance*`
    /// - offset: 24
    /// -  flags: `FLAGS_NONE | NOT_OWNED`
    #[serde(rename = "constraintInstance")]
    ConstraintInstance(Cow<'a, str>),
}

// Manual implementation to branch the process using the value of the `name` attribute as the key.
impl_deserialize_for_internally_tagged_enum! {
    HkpConstraintChainInstanceAction<'de>, "@name",
    ("constraintInstance" => ConstraintInstance(Cow<'de, str>)),
}
