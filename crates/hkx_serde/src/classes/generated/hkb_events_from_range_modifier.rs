...
pub struct HkbEventsFromRangeModifier<'a> {
    ...
    /// - offset: 52
    /// -  flags: `FLAGS_NONE`
    event_ranges: Cow<'a, str>,
    /// # C++ Class Fields Info
    /// -   name:`"wasActiveInPreviousFrame"`
    /// -   type: `hkArray<void>`
    /// - offset: 56
    /// -  flags: `FLAGS_NONE|SERIALIZE_IGNORED`
    was_active_in_previous_frame: HkArrayRef<()>,
}
...
impl ByteDeSerialize for HkbEventsFromRangeModifier<'_> {
    fn from_bytes<B>(
        _bytes: &[u8],
...
#[serde(tag = "@name")]
pub enum HkbEventsFromRangeModifierVisitor<'a> {
    /// Visitor fields
    #[serde(rename = "enable")]
