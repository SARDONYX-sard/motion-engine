//! Rust [`serde::Serializer`]/[`serde::Deserializer`] corresponding to C++ class `hkpCharacterMotion`
//!
//! # NOTE
...
impl Serialize for HkpCharacterMotion<'_> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        // Use `Vec` instead, because the fields of this class are more than 32 and serde only supports up to `[T; 32]`.
        let visitor: Vec<HkpCharacterMotionVisitor<'_>> = self.into();
        visitor.serialize(serializer)
    }
}

impl<'de> Deserialize<'de> for HkpCharacterMotion<'de> {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        // Use `Vec` instead, because the fields of this class are more than 32 and serde only supports up to `[T; 32]`.
        let de = <Vec<HkpCharacterMotionVisitor<'de>>>::deserialize(deserializer)?;
        Ok(de.into())
    }
}

impl<'a> From<Vec<HkpCharacterMotionVisitor<'a>>> for HkpCharacterMotion<'a> {
    fn from(_values: Vec<HkpCharacterMotionVisitor<'a>>) -> Self {
            let mut _type = None;
        ...
        for _value in _values {
            match _value {
                HkpCharacterMotionVisitor::Type(m) => _type = Some(m),
                HkpCharacterMotionVisitor::DeactivationIntegrateCounter(m) => deactivation_integrate_counter = Some(m),
                HkpCharacterMotionVisitor::DeactivationNumInactiveFrames(m) => deactivation_num_inactive_frames = Some(m),
                HkpCharacterMotionVisitor::MotionState(m) => motion_state = Some(m),
                HkpCharacterMotionVisitor::InertiaAndMassInv(m) => inertia_and_mass_inv = Some(m),
                HkpCharacterMotionVisitor::LinearVelocity(m) => linear_velocity = Some(m),
                HkpCharacterMotionVisitor::AngularVelocity(m) => angular_velocity = Some(m),
                HkpCharacterMotionVisitor::DeactivationRefPosition(m) => deactivation_ref_position = Some(m),
                HkpCharacterMotionVisitor::DeactivationRefOrientation(m) => deactivation_ref_orientation = Some(m),
                HkpCharacterMotionVisitor::SavedMotion(m) => saved_motion = Some(m),
                HkpCharacterMotionVisitor::SavedQualityTypeIndex(m) => saved_quality_type_index = Some(m),
                HkpCharacterMotionVisitor::GravityFactor(m) => gravity_factor = Some(m),
                HkpCharacterMotionVisitor::MemSizeAndFlags(m) => mem_size_and_flags = Some(m),
                HkpCharacterMotionVisitor::ReferenceCount(m) => reference_count = Some(m),
...
impl<'a> From<&HkpCharacterMotion<'a>> for Vec<HkpCharacterMotionVisitor<'a>> {
    fn from(data: &HkpCharacterMotion<'a>) -> Self {
        vec![
            HkpCharacterMotionVisitor::Type(data._type.clone().into()),
            HkpCharacterMotionVisitor::DeactivationIntegrateCounter(data.deactivation_integrate_counter.into()),
            HkpCharacterMotionVisitor::DeactivationNumInactiveFrames(data.deactivation_num_inactive_frames.clone()),
            HkpCharacterMotionVisitor::MotionState(data.motion_state.clone()),
            HkpCharacterMotionVisitor::InertiaAndMassInv(data.inertia_and_mass_inv.into()),
            HkpCharacterMotionVisitor::LinearVelocity(data.linear_velocity.into()),
            HkpCharacterMotionVisitor::AngularVelocity(data.angular_velocity.into()),
            HkpCharacterMotionVisitor::DeactivationRefPosition(data.deactivation_ref_position.clone()),
            HkpCharacterMotionVisitor::DeactivationRefOrientation(data.deactivation_ref_orientation.clone()),
            HkpCharacterMotionVisitor::SavedMotion(data.saved_motion.clone().into()),
            HkpCharacterMotionVisitor::SavedQualityTypeIndex(data.saved_quality_type_index.into()),
            HkpCharacterMotionVisitor::GravityFactor(data.gravity_factor.into()),
            HkpCharacterMotionVisitor::MemSizeAndFlags(data.mem_size_and_flags.into()),
            HkpCharacterMotionVisitor::ReferenceCount(data.reference_count.into()),

        ]
    }
}

impl <'bytes: 'de, 'de> ByteDeSerialize<'bytes, 'de> for HkpCharacterMotion<'de> {
    fn from_bytes<B>(
        _bytes: &'bytes [u8],
...
#[serde(tag = "@name")]
enum HkpCharacterMotionVisitor<'a> {
    /// Visitor fields
    #[serde(rename = "type")]
...
impl_deserialize_for_internally_tagged_enum! {
    HkpCharacterMotionVisitor<'de>, "@name",
